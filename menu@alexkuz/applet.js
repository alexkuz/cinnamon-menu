...
class CinnamonEnhancedMenuApplet extends Applet.TextIconApplet {
    constructor(orientation, panel_height, instance_id) {
        super(orientation, panel_height, instance_id);

        this._updatePanelSizes();
        global.settings.connect('changed::panels-height', Lang.bind(this, this._panelHeightChanged));
        ...
        this.settings = new Settings.AppletSettings(this, "menu@alexkuz", instance_id);
        ...
        this.settings.bind("menu-icon-padding", "menuIconPadding", this._updateIconAndLabel);
        this.settings.bind("match-panel-size", "matchPanelSize", this._updateIconAndLabel);
    ...
    _updatePanelSizes() {
        var pm = Main.panelManager;
        var panels = pm.getPanelsInMonitor(0);

        var panelNames = ['top', 'bottom', 'left', 'right'];
        
        this._panelSizes = panels.reduce((acc, panel) => {
            acc[panelNames[panel.panelPosition]] = panel.height;
            return acc;
        }, {});
    }
    ...
    _setStyle() {
        ...
        let isCurrentSidePanel = this._orientation == St.Side.LEFT || this._orientation == St.Side.RIGHT;
        if (this.matchPanelSize != "none" && this._panelSizes[this.matchPanelSize]) {
            let matchSize = this._panelSizes[this.matchPanelSize];
            let isMatchingSidePanel = this.matchPanelSize == "left" || this.matchPanelSize == "right";

            if (isCurrentSidePanel !== isMatchingSidePanel) {
                if (isCurrentSidePanel) {
                    padding = [(matchSize - size * global.ui_scale - 16) / 2 / global.ui_scale, 0];
                } else {
                    padding = [0, (matchSize - size * global.ui_scale - 16) / 2 / global.ui_scale];
                }
            }
        } else if (this.menuIconPadding > 0) {
            if (isCurrentSidePanel) {
                padding = [this.menuIconPadding, 0];
            } else {
                padding = [0, this.menuIconPadding];
            }
        }

        this._applet_icon.set_style("padding: " + padding[0] + "px " + padding[1] + "px;");
    ...
    _getNewVectorInfo() {
        ...
        if (CinnamonEnhancedMenuApplet.DEBUG_VMASK) {
    ...
    _keepMaskActive() {
        ...
        if (Math.abs(Math.hypot(x_dist, y_dist)) < CinnamonEnhancedMenuApplet.MIN_MOVEMENT) {
            return false;
        }
    ...
    _enableVectorMask(actor) {
        ...
        this.vector_update_loop = Mainloop.timeout_add(CinnamonEnhancedMenuApplet.POLL_INTERVAL, Lang.bind(this, this._maskPollTimeout));
    }
...
function main(metadata, orientation, panel_height, instance_id) {
    return new CinnamonEnhancedMenuApplet(orientation, panel_height, instance_id);
}
